...
class DeepAgentWorkerAgentRunnable(_WorkerRunnableBase):
    def __init__(self, model_name: str, artifact_dir: Path) -> None:
        ...
        def make_backend(runtime: object) -> object:
            return CompositeBackend(
                default=StateBackend(runtime),
                routes={"/memories/": StoreBackend(runtime), "/artifacts/": artifacts_backend},
            )

        def write_artifact_via_backend(payload_text: str) -> str:
            filename = f"artifact_{int.from_bytes(os.urandom(6), 'big')}.json"
            write_result = cast(_WriteResult, artifacts_backend.write(f"/{filename}", payload_text))
...
def build_worker_agent_runnable(use_real_agent: bool, backend: str, model_name: str, artifact_dir: Path) -> WorkerAgentRunnable:
    if not use_real_agent:
        return EchoWorkerAgentRunnable()
    if backend == "deepagent":
        return DeepAgentWorkerAgentRunnable(model_name=model_name, artifact_dir=artifact_dir)
